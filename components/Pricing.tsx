...
import { Check, Loader2, Building2, User, Star, CreditCard, Shield, EyeOff } from 'lucide-react';
...
const Pricing: React.FC<PricingProps> = ({ onSignup }) => {
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {/* Pricing Grid */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start relative z-10 max-w-7xl mx-auto">

                {/* 1. FREE TIER (The Hook) */}
                <div className="p-6 border border-white/5 bg-slate-900/40 backdrop-blur-md rounded-2xl transition-all relative group hover:-translate-y-1 duration-300">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="p-3 bg-slate-800/50 rounded-xl border border-white/5">
                            <EyeOff className="w-5 h-5 text-slate-400" />
                        </div>
                        <div>
                            <h3 className="text-lg font-mono font-bold text-slate-200 uppercase tracking-widest">Free</h3>
                            <div className="text-[10px] text-slate-500 uppercase tracking-widest mt-1">Product-Led Growth</div>
                        </div>
                    </div>

                    <div className="flex items-baseline gap-1 mb-8">
                        <span className="text-4xl font-black text-slate-200 tracking-tighter">$0</span>
                    </div>

                    <ul className="space-y-4 mb-8">
                        {[
                            "3 Scope Checks / Month",
                            "Watermarked Output",
                    ...
                    <button
                        ...
                        disabled={!!processing}
                        className="w-full py-3 bg-slate-800/50 text-slate-400 rounded-xl font-bold font-mono uppercase tracking-wider text-xs hover:bg-slate-800 hover:text-white transition-all border border-slate-700/30"
                    >
                        {processing === 'free' ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Join Waitlist"}
                    </button>
                </div>

                {/* 2. FREELANCER TIER (The Impulse Buy) */}
                <div className="p-6 border border-indigo-500/20 bg-slate-900/60 backdrop-blur-md rounded-2xl transition-all relative group hover:-translate-y-1 duration-300 shadow-[0_0_20px_rgba(79,70,229,0.1)]">
                    <div className="absolute top-0 right-0 p-4">
                        <span className="bg-slate-800 text-slate-300 text-[9px] font-bold px-2 py-1 rounded border border-slate-700">SOLO</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h3 className="text-lg font-mono font-bold text-white uppercase tracking-widest">Freelancer</h3>
                            <div className="text-[10px] text-indigo-400 uppercase tracking-widest mt-1">Impulse Buy</div>
                        </div>
                    </div>
                    ...
                    <p className="text-[10px] text-slate-500 mb-8 font-mono">
                        {billingCycle === 'monthly' ? 'Billed monthly' : 'Equivalent to $15/mo'}
                    </p>

                    <ul className="space-y-4 mb-8">
                        {[
                            { text: "Unlimited Email Drafts", highlight: true },
                            { text: "3 Active Rulebooks", highlight: true },
                            { text: "Chrome Extension", highlight: true },
                            { text: "Text Only (No PDF)", highlight: false, dim: true }
                        ].map((item, i) => (
                            <li key={i} className={`flex items-start gap-3 text-sm ${item.dim ? 'text-slate-600' : 'text-slate-300'}`}>
                                <div className={`p-0.5 rounded-full ${item.highlight ? 'bg-indigo-500/20' : 'bg-slate-800'}`}>
                                    <Check className={`w-3 h-3 ${item.highlight ? 'text-indigo-400' : 'text-slate-600'}`} />
                                </div>
                                <span className={item.highlight ? "text-slate-200" : ""}>{item.text}</span>
                            </li>
                        ))}
                    </ul>

                    <button
                        onClick={() => handleSubscribe('freelancer')}
                        disabled={!!processing}
                ...
                {/* 3. AGENCY TIER (Revenue Recovery - Holographic) */}
                <div className="relative p-[1px] rounded-2xl overflow-hidden group hover:-translate-y-2 transition-transform duration-500">
                    {/* Animated Gradient Border */}
                    <div className="absolute inset-0 bg-gradient-to-r from-emerald-500 via-purple-500 to-emerald-500 animate-[gradient-x_3s_linear_infinite] opacity-70"></div>
                    ...
                    <div className="relative h-full bg-slate-950/90 backdrop-blur-xl rounded-2xl p-6 overflow-hidden">
                        ...
                        <div className="absolute top-0 right-0 p-4">
                            <span className="bg-emerald-500 text-white text-[10px] font-bold px-2 py-1 rounded shadow-[0_0_10px_rgba(16,185,129,0.5)]">BEST VALUE</span>
                        </div>

                        <div className="flex items-center gap-3 mb-6 relative z-10">
                            <div className="p-3 bg-gradient-to-br from-emerald-500/20 to-purple-500/20 rounded-xl border border-emerald-500/30">
                                <Building2 className="w-5 h-5 text-emerald-400" />
                            </div>
                            <div>
                                <h3 className="text-lg font-mono font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-emerald-200 uppercase tracking-widest">Agency</h3>
                                <div className="text-[10px] text-emerald-400 uppercase tracking-widest mt-1">Revenue Recovery</div>
                            </div>
                        </div>

                        <div className="flex items-baseline gap-1 mb-2 relative z-10">
                            <span className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-slate-300 tracking-tighter">
                                ${billingCycle === 'monthly' ? '149' : '1,500'}
                            </span>
                            <span className="text-sm text-slate-500 font-normal uppercase tracking-widest">
                                /{billingCycle === 'monthly' ? 'mo' : 'yr'}
                            </span>
                        </div>
                        <p className="text-[10px] text-slate-500 mb-8 font-mono relative z-10">
                            {billingCycle === 'monthly' ? 'Recover 1 billable hour to pay for this' : 'Save $288/yr'}
                        </p>
                        ...
                        <button
                            ...
                            disabled={!!processing}
                            className="relative w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl font-bold font-mono uppercase tracking-wider text-xs hover:scale-[1.02] transition-all shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:shadow-[0_0_50px_rgba(16,185,129,0.5)] border border-white/10 z-10 overflow-hidden"
                        >

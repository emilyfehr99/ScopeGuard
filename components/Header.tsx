...
import { Shield, Activity, Zap, Menu, X } from 'lucide-react';
...
const Header: React.FC<HeaderProps> = ({ credits, currentView, onNavigate, onLogin, onLogout, user }) => {
            ...
            <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between relative z-10">
                ...
                <nav className="hidden md:flex items-center gap-8">
                    ...
                    <button
                        ...
                        className={`text-xs font-mono font-medium uppercase tracking-widest transition-colors ${currentView === 'pricing' ? 'text-indigo-400' : 'text-slate-400 hover:text-white'}`}
                    >
                        Pricing
                    </button>
                </nav>
                ...
                <div className="flex items-center gap-6">
                    ...
                    {currentView === 'app' && (
                        <div className="hidden lg:flex items-center gap-6 border-r border-slate-800 pr-6 mr-2">
                        ...
                        {currentView !== 'app' ? (
                            <>
                                {user ? (
                                    <button
                                        onClick={onLogout}
                                        className="hidden sm:block text-xs font-mono font-medium text-slate-400 hover:text-rose-400 transition-colors uppercase tracking-widest"
                                    >
                                        Sign Out
                                    </button>
                                ) : (
                                ...
                                <button
                                    onClick={() => onNavigate('pricing')}
                                    className="hidden sm:block px-5 py-2 bg-white text-slate-950 hover:bg-indigo-50 hover:scale-105 transition-all uppercase rounded font-bold text-[10px] tracking-wider shadow-[0_0_20px_rgba(255,255,255,0.2)]"
                                >
                                    Get Started
                                </button>
                            </>
                            ...
                            <>
                                {credits > 50 ? (
                                    <span className="text-indigo-400 uppercase tracking-wider font-bold drop-shadow-[0_0_10px_rgba(129,140,248,0.3)] text-[10px]">PRO ACCOUNT</span>
                                ) : (
                                    <button
                                        onClick={() => onNavigate('pricing')}
                                        className="hidden sm:block hover:text-indigo-400 transition-colors uppercase tracking-widest text-[10px] font-mono text-slate-400"
                                    >
                                        Upgrade
                                    </button>
                                )}
                                <button
                                    ...
                                    className="hidden sm:block px-4 py-2 bg-indigo-600/10 border border-indigo-500/50 text-indigo-400 hover:bg-indigo-600 hover:text-white hover:border-indigo-500 hover:shadow-[0_0_15px_rgba(99,102,241,0.3)] transition-all uppercase rounded font-bold tracking-wider text-[10px]"
                                >
                ...
                <div className="fixed inset-0 top-20 bg-slate-950/95 backdrop-blur-xl z-40 md:hidden animate-in slide-in-from-top-10 flex flex-col p-6 border-t border-white/10">
                    <nav className="flex flex-col gap-6 items-center pt-8">
                        ...
                        <button
                            ...
                            className={`text-lg font-mono font-medium uppercase tracking-widest transition-colors ${currentView === 'pricing' ? 'text-indigo-400' : 'text-slate-300 hover:text-white'}`}
                        >
                            Pricing
                        </button>
                        ...
                        {user ? (
                            <button
                                ...
                                className="text-lg font-mono font-medium text-rose-400 hover:text-rose-300 transition-colors uppercase tracking-widest"
                            >
                                Sign Out
                            </button>
                        ) : (
                            <button
                                ...
                                className="text-lg font-mono font-medium text-white hover:text-indigo-400 transition-colors uppercase tracking-widest"
                            >
                                Login
                            </button>
                        )}
                        ...
                        <button
                            onClick={() => { onNavigate('pricing'); setIsMobileMenuOpen(false); }}
                            className="w-full max-w-xs px-6 py-4 bg-white text-slate-950 hover:bg-indigo-50 transition-all uppercase rounded font-bold text-sm tracking-wider shadow-lg mt-4"
                        >
                            Get Started
                        </button>
                    </nav>
